use std::cmp::Reverse;
use std::iter::{self, Peekable, from_fn};
use std::mem::take;
use std::sync::Arc;
    ...
    from_fn(move || {
        loop {
            ...
            while let Some(excluded_range) = &current_excluded_range
                && excluded_range.end <= range.start
            {
            ...
            if let Some(excluded_range) = &current_excluded_range
                && ranges_intersect(&range, excluded_range)
            {
        ...
        loop {
                ...
                && parent_range.end <= range.start
            {
                ...
                if let Some(next_parent_range) = &mut next_parent_range
                    && let Some(next_next_range) = parent_ranges.peek()
                    && next_parent_range.end == next_next_range.start
                {
